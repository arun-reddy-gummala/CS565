...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # if the noisyDistance is None then the ghost has been captured by the pacman
        if noisyDistance == None:
            allPossible[self.getJailPosition()] = 1.0

        else:
            for location in self.legalPositions:
                # calculating the manhattan distance from the possible ghost position to the pacman position
                true_distance = util.manhattanDistance(location, pacmanPosition)
                # updating beliefs based on the distance observation and Pacman's position
                allPossible[location] = emissionModel[true_distance] * self.beliefs[location]

        "*** END YOUR CODE HERE ***"

        # normalizing all the obtained values
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # to obtain the distribution over new positions for the ghost,
        #         given its previous position (oldPos) as well as Pacman's current position
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            # accessing the items in new position
            for newPos, prob in newPosDist.items():
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # initializing to store all legalpositions in a list
        self.particles = []

        for i in range(self.numParticles):
            j = i % len(self.legalPositions)
            # evenly distributing particles across various positions
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            self.particles = []
            for i in range(self.numParticles):
                # updating all the particles so that the ghost appears in its prison cell
                self.particles.append(self.getJailPosition())
        else:
            nonZero = False
            weights = util.Counter()
            for particle in self.particles:
                # to calculate the distance between a particle and Pacman's position
